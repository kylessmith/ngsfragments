...
my_include_dirs = [".", np.get_include(), ailist.get_include(), "htslib"]
...
def declare_cython_extension(extName, use_math=False, use_openmp=False, include_dirs=None, macros=None, sources=[], extra_libraries=[], extra_compile_args=[]):
    """
    ...
    if use_math:
        compile_args = list(my_extra_compile_args_math) + extra_compile_args # copy
        link_args    = list(my_extra_link_args_math) + extra_compile_args
        libraries    = ["m"]  # link libm; this is a list of library names without the "lib" prefix
    else:
        compile_args = list(my_extra_compile_args_nonmath) + extra_compile_args
        link_args    = list(my_extra_link_args_nonmath) + extra_compile_args
...
extra_libraries = ['z', 'bz2', 'lzma', 'curl', 'ssl'] + (['crypt'] if platform.system() != 'Darwin' else [])
extra_compile_args=["-Wno-sign-compare", "-Wno-unused-function", "-Wno-strict-prototypes", "-Wno-unused-result", "-Wno-discarded-qualifiers", "-Wno-ignored-qualifiers"]

# declare Cython extension modules here
#ext_module_fragments = declare_cython_extension( "ngsfragments.fragments", use_math=False, use_openmp=True, include_dirs=my_include_dirs )
ext_module_read_sam = declare_cython_extension( "ngsfragments.read_sam.ReadSam", use_math=False, use_openmp=False , include_dirs=my_include_dirs, sources=sources, extra_libraries=extra_libraries, extra_compile_args=extra_compile_args)
ext_module_median = declare_cython_extension( "ngsfragments.peak_calling.RunningMedian", use_math=False, use_openmp=False, include_dirs=my_include_dirs)
ext_module_peaks = declare_cython_extension( "ngsfragments.peak_calling.CallPeaks", use_math=False, use_openmp=False , include_dirs=my_include_dirs)
ext_module_smooth = declare_cython_extension( "ngsfragments.segment.smooth_cnv.smooth_cnv", use_math=False, use_openmp=False , include_dirs=my_include_dirs)

# this is mainly to allow a manual logical ordering of the declared modules
cython_ext_modules = [ext_module_read_sam, ext_module_median, ext_module_peaks, ext_module_smooth]
...
setup(
    ...
    packages = ["ngsfragments", "ngsfragments.read_sam", "ngsfragments.segment", "ngsfragments.plot"],
    package_data={'ngsfragments': ['*.pxd', '*.pyx', '*.c', '*.h']},
    # Disable zip_safe
    zip_safe = False,
    # Custom data files not inside a Python package
