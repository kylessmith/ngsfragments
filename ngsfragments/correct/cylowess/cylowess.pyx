...
def lowess(np.ndarray[DTYPE_t, ndim = 1] endog,
    ...
    cdef:
        ...
        np.ndarray[np.int_t, ndim = 1] original_index
    ...
    # Return the fit values in the original order of the input x values.
    original_index = np.zeros(len(x), dtype = np.int_)
    for i in range(len(x)):
        original_index[sort_index[i]] = i
    y_fit = y_fit[original_index]
    #x = x[original_index]
